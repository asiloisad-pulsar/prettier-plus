...
const refreshGitIndexForFiles = async (filePaths) => {
  ...
  for (const [repositoryPath, relativePaths] of filesByRepository) {
    try {
      // `git add --refresh` rejects untracked pathspecs, so keep only index entries.
      const { stdout } = await execGit(
        [
          "--literal-pathspecs",
          "-c",
      ...
      // Unlike `update-index --refresh` followed by paths, this does not stage file contents.
      await execGit(
        [
          "--literal-pathspecs",
          "-c",
      ...
      log("Refreshed git index metadata:", repositoryPath, trackedPaths.length, "file(s)");
    } catch (error) {
